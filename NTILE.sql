...
NTILE(2) OVER(PARTITION BY account_id ORDER BY standard_qty) AS gloss_half,
CASE WHEN NTILE(2) OVER(PARTITION BY account_id ORDER BY standard_qty) = 1 THEN 'Level 1'
     WHEN NTILE(2) OVER(PARTITION BY account_id ORDER BY standard_qty) = 2 THEN 'Level 2'
     END AS gloss_level
FROM orders
ORDER by account_id;
